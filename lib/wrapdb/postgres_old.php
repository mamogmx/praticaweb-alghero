...
class sql_db
{
	...
	function sql_fetchrow_array($query_id = 0)
	{
		if( !$query_id )
		{
			$query_id = $this->query_result;
		...
		if($query_id)
		{
			...
			if( $this->row )
			{
				for($i=0;$i<count($this->row);$i++){
					if (substr($this->sql_fieldtype($i),0,1)=="_" AND $this->row[$this->sql_fieldname($i)]) {
						$cmd="\$tmp[".$this->sql_fieldname($i)."]=".str_replace("}",")",str_replace("{","Array(",$this->row[$this->sql_fieldname($i)])).";";
						eval($cmd);
					}
					else
						$tmp[$this->sql_fieldname($i)]=$this->row[$this->sql_fieldname($i)];
				} 
				$this->rownum[$query_id]++;
				return $tmp;
			}
	...
	function sql_fetchrowset_array($query_id = 0)
	{			
		if( !$query_id )
		{
			$query_id = $this->query_result;
		...
		if( $query_id )
		{
			...
			{				
				...
				for($i=0;$i<count($this->rowset);$i++){
					if (substr($this->sql_fieldtype($i),0,1)=="_" and $this->rowset[$this->sql_fieldname($i)]){
						$cmd="\$tmp=".str_replace("}",")",str_replace("{","Array(",str_replace(",",",",$this->rowset[$this->sql_fieldname($i)]))).";";
						eval($cmd);
						$result[$j][$this->sql_fieldname($i)]=$tmp;
					}
					else
						$result[$j][$this->sql_fieldname($i)]=$this->rowset[$this->sql_fieldname($i)];
				}
				$this->rownum[$query_id]++;
				$j++;
			}
