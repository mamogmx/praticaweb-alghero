<html>
	...
	<body>
		<div style="width:18cm">
			<div style="float:right; font-weight:bold; font-size:8pt; text-align:right">Mod. AP/8<br>Art. 45 del Regol.</div>
			<div style="font-style:italic">FAC-SIMILE</div>
		</div>
		<table style="font-size:9px; border:2px solid black; width:18cm">
			<caption style="text-align:center; font-weight:bold; font-size:16pt">STRADARIO</caption>
			<tr style="background-color:rgb(240,240,255)">
				...
				<td rowspan="2" valign="center" align="center"><b>UBICAZIONE DELL'AREA<br>DI CIRCOLAZIONE</b></td>
			...
			$sql_1="
			SELECT tpstrid, initcap(substring(tpstrnom,1,position(' ' in tpstrnom)-1)) as specie,
				substring(
				tpstrnom,
				length(substring(tpstrnom,1,position(' ' in tpstrnom)-1))+2,
				length(tpstrnom) - length(substring(tpstrnom,1,position(' ' in tpstrnom)-1))
				) as denominazione, tpstrnom
				FROM dbt_topociv.dbt_tpstr
				WHERE tpstrid IN (SELECT tpstrid FROM dbt_topociv.dbt_civico)
				ORDER BY 3;";
